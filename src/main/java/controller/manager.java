...
public class Manager {
    ...
    synchronized public List<Account> getAllAccounts()  {
        Session session = null;
    ...
    synchronized public void deleteAccount(int account)  {
        Session session = null;
        try {
            session = getSessionFactory().openSession();
            session.beginTransaction();
    ...
    synchronized public boolean updateAccount(int account, int quantity)
    {
        ...
        if (state + quantity >= 0) {
            ...
            try {
                ...
                account1.setState(state + quantity);
                session = getSessionFactory().openSession();
                session.beginTransaction();
    ...
    synchronized public boolean makeTransaction(int accountFrom, int accountTo, int quantity)  {
        ...
        if (accountFrom != accountTo) {
            if (updateAccount(accountFrom, -quantity)) {
                updateAccount(accountTo, quantity);
                return true;
            }
            else return false;
        }
